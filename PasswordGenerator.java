...
public class PasswordGenerator
 {
    ...
    public static String generateComplexPassword(int length, boolean useUppercase, boolean useLowercase, boolean useDigits, boolean useSymbols) {
        StringBuilder characters = new StringBuilder();
        if (useUppercase) characters.append(CHARACTERS_UPPER);
        if (useLowercase) characters.append(CHARACTERS_LOWER);
        if (useDigits) characters.append(CHARACTERS_DIGITS);
        if (useSymbols) characters.append(CHARACTERS_SYMBOLS);

        if (characters.length() == 0) throw new IllegalArgumentException("At least one character set must be selected.");
    ...
    public static String assessPasswordStrength(String password) {
        ...
        if (totalScore >= 9) return "Very Strong";
        if (totalScore >= 7) return "Strong";
    ...
    public static void main(String[] args) {
        ...
        try {
            String originalPassword = generateComplexPassword(length, useUppercase, useLowercase, useDigits, useSymbols);
